...
if __name__ == "__main__":
    ...
    for group, data in bsm_list.items():
        if not site_bsms.get(data["parent"]):
            site_bsms[data["parent"]] = my_server.bsm.find_bsm_name(name=data["parent"])
        if not site_bsms.get(data["parent"]):
            ...
            for ip in node["node"].ipInterfaces:
                for service in ip.services:
                    if (
                        ...
                        if node["function"] == "S":
                            if service.id in [
                                edge.ip_service_id for edge in new_bsm.ip_service_edges
                            ]:
                                new_bsm.ip_service_edges.remove(
                                    [
                                        edge
                                        for edge in new_bsm.ip_service_edges
                                        if edge.ip_service_id == service.id
                                    ][0]
                                )
                            new_bsm.ip_service_edges.append(
                                IPServiceEdgeRequest(
                            ...
                            if service.id in [
                                edge.ip_service_id for edge in vnf_bsm.ip_service_edges
                            ]:
                                vnf_bsm.ip_service_edges.remove(
                                    [
                                        edge
                                        for edge in vnf_bsm.ip_service_edges
                                        if edge.ip_service_id == service.id
                                    ][0]
                                )
                            vnf_bsm.ip_service_edges.append(
                                IPServiceEdgeRequest(
                                    friendly_name=friendly_name,
                                    ip_service_id=service.id,
                                )
        ...
        # if not new_vnf_bsm:
        #    new_vnf_bsm = my_server.bsm.find_bsm_name(name=vnf_bsm.name)
        #    new_vnf_bsm = my_server.bsm.update_bsm(new_vnf_bsm.id, vnf_bsm)
        ...
        # if not new_parent_bsm:
        #    new_parent_bsm = my_server.bsm.find_bsm_name(name=new_bsm.name)
        #    new_parent_bsm = my_server.bsm.update_bsm(new_parent_bsm.id, new_bsm)
        ...
        site_bsm_update.child_edges.append(
            ChildEdgeRequest(
