...
CRITICAL_FUNCTIONS = ["S", "SAOS"]
MAJOR_FUNCTIONS = ["DNFVI", "DNFV"]
MINOR_FUNCTIONS = ["VMR", "VCE"]
WARNING_FUNCTIONS = ["EDGE", "RTR01"]

FUNCTIONS = CRITICAL_FUNCTIONS + MAJOR_FUNCTIONS + MINOR_FUNCTIONS + WARNING_FUNCTIONS
...
def generate_bsm_list(
    ...
    for node in tqdm(
        ...
        if not node.assetRecord.displayCategory:
            continue
        match = re.match(
            f"MI_(?P<org>.*)-(?P<host>.*)(?P<instance>[0-9][0-9])(?P<function>{'|'.join(FUNCTIONS)})",
            node.label,
        )
